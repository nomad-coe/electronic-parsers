...
class OutParser(TextParser):
    ...
    def init_quantities(self):
        ...
        dft_quantities = geometry_quantities + [
            ...
            Quantity(
                ...
                str_operation=lambda x: [
                    [vi.strip() for vi in v.split(':')] for v in x.split('\n')
                ],
                convert=False,
            ),
            Quantity(
            ...
            Quantity(
                'time_calculation',
                rf'Total iterative time = +({re_float})s',
                dtype=np.float64,
                unit=ureg.s,
            ),
        ]
        ...
        dft_gradient_quantities = [
            ...
            Quantity('energy', r'Step\s*Energy.+[\-\s\@]+(.+)'),
        ]
        ...
        pw_quantities = geometry_quantities + [
            ...
            Quantity('spin_S2', rf'\<S\^2\>\s*\=\s*({re_float})', dtype=np.float64),
            Quantity(
                'parameters',
                r'options\:\s*([\s\S]+?)\n *\n',
                str_operation=lambda x: [
...
class NWChemParser:
    def __init__(self):
        ...
        self._metainfo_map = {
            'hostname': 'run_host_name',
            'program': 'program_name',
            'date': 'start_datetime',
            'compiled': 'compilation_datetime',
            'nwchem branch': 'branch',
            'nwchem revision': 'revision',
            'input': 'input_filename',
            'prefix': 'input_prefix',
            'data base': 'db_filename',
            'Charge': 'total_charge',
            ...
            'Total DFT energy': 'energy_total',
            'One electron energy': 'x_nwchem_energy_one_electron',
            'Coulomb energy': 'energy_coulomb',
            'Exchange energy': 'energy_exchange',
            'Exchange-Corr. energy': 'energy_XC',
            'Correlation energy': 'energy_correlation',
            'Nuclear repulsion energy': 'energy_nuclear_repulsion',
            'total     energy': 'energy_total',
            'exc-corr  energy': 'energy_XC',
            ...
            'Target temp. (K)': 'qmd_target_temperature',
            'Thermostat': 'qmd_thermostat',
            'Tau': 'qmd_tau',
            'Random seed': 'qmd_random_seed',
            'Nuclear integrator': 'qmd_nuclear_integrator',
            'Current temp. (K)': 'qmd_initial_temperature',
    ...
    def parse_system(self, source):
        ...
        if len(positions) == 0:
            ...
            labels_positions = dft.get(
                'labels_positions_forces', dft_gradient.get('labels_positions_forces')
            )
            if labels_positions is not None:
                labels, positions = labels_positions[:2]

        if len(positions) == 0:
            labels, positions = self.out_parser.get('input', {}).get(
                'labels_positions', [[], []]
            )
    ...
    def parse_scc(self, source):
        ...
        # forces
        forces = dft_gradient.get(
            'labels_positions_forces', dft.get('labels_positions_forces')
        )
        ...
        if qmd_info is not None:
            ...
            for key, val in qmd_info:
                ...
                if key == 'Time elapsed (fs)':
                    ...
                    sec_qmd.x_nwchem_qmd_step_kinetic_energy = (
                        float(val[1]) * ureg.hartree
                    )
                elif key == 'Pot. energy (a.u.)':
                    sec_qmd.x_nwchem_qmd_step_potential_energy = (
                        float(val[1]) * ureg.hartree
                    )
                elif key == 'Tot. energy (a.u.)':
                    sec_qmd.x_nwchem_qmd_step_total_energy = (
                        float(val[1]) * ureg.hartree
                    )
                elif key == 'Target temp. (K)':
                    sec_qmd.x_nwchem_qmd_step_target_temperature = (
                        float(val[1]) * ureg.kelvin
                    )
        ...
        if scf is not None:
            for iteration in scf.get('iteration', []):
                ...
                iteration = [
                    fix_dfloat(i) if isinstance(i, str) else i for i in iteration
                ]
    ...
    def parse_method(self, source):
            ...
            for f_a in functionals:
                ...
                for f_b in functionals:
                    if len(f_a) < 3 or len(f_b) < 3:
                        continue
                    combination = '+'.join(
                        ['%s_%1.3f_%s' % tuple(f) for f in [f_a, f_b]]
                    )
        ...
        sec_basis = BasisSet()
        sec_basis.type = (
            'plane waves' if self.out_parser.get('pw') is not None else 'gaussians'
        )
        sec_basis.scope = (
            ['valence'] if sec_basis.type == 'gaussians' else []
        )  # TODO: add distinction for ECP
        full_type = (
            'plane waves'
            if sec_basis.type == 'plane waves'
            else 'atom-centered orbitals'
        )
    ...
    def parse_configurations(self):
        ...
        if self.out_parser.get('geometry_optimization') is not None:
            for iteration in self.out_parser.get('geometry_optimization').get(
                'iteration', []
            ):
                parse_calculation(iteration)

        elif self.out_parser.get('molecular_dynamics') is not None:
            for iteration in self.out_parser.get('molecular_dynamics').get(
                'iteration', []
            ):
                parse_calculation(iteration)
    ...
    def parse_workflow(self):
        ...
        parameters = []
        if self.out_parser.get('geometry_optimization') is not None:
            parameters = self.out_parser.get('geometry_optimization').get(
                'parameters', []
            )
            ...
            workflow = (
                GeometryOptimization()
                if len(self.out_parser.get('pw')) > 1
                else SinglePoint()
            )
    ...
    def parse(self, filepath, archive, logger):
        ...
        sec_run.program = Program(
            name='NWChem', version=self.out_parser.get('version', '')
        )
