                    ...
                    quantities=[
                        ...
                        Quantity(
                            ...
                            str_operation=lambda x: [
                                v.split()[2] for v in x.strip().splitlines()
                            ],
                            unit=1 / ureg.cm,
                        ),
                        Quantity(
    ...
    def parse(self, filepath, archive, logger):
        ...
        def parse_calculation(source, target=None):
            ...
            if source.vibrational_frequencies is not None:
                ...
                if source.normal_modes is not None:
                    ...
                    sec_vibrations.x_dmol3_normal_modes = np.transpose(
                        np.reshape(value, (n_atoms, 3, len(value[0]))), axes=(2, 0, 1)
                    )
        ...
        if self.mainfile_parser.vibrations is not None:
            ...
            if thermo_data is not None:
                ...
                workflow.results.entropy = (
                    thermo_data[2] * ureg.J * 4.184 / ureg.K / MOL
                )
                workflow.results.heat_capacity_c_p = (
                    thermo_data[3] * ureg.J * 4.184 / ureg.K / MOL
                )
                workflow.results.enthalpy = thermo_data[4] * ureg.J * 4184.0 / MOL
                workflow.results.gibbs_free_energy = (
                    thermo_data[5] * ureg.J * 4184.0 / MOL
                )
