...
class OutParser(TextParser):
    ...
    def init_quantities(self):
        ...
        calc_quantities = [
            ...
            Quantity(
                'time_physical',
                rf'TOTAL WALL CLOCK TIME= *({re_f}) SECONDS',
                repeats=True,
            ),
            Quantity(
                'coordinates',
        ...
        self._quantities = [
            ...
            Quantity(
                'x_gamess_memory', r'(\d+) WORDS OF MEMORY AVAILABLE', dtype=np.int32
            ),
            Quantity(
                'basis_options',
...
class GamessParser:
    def __init__(self):
        ...
        self._xc_map = {
            ...
            'CAMB': [
                {'name': 'GGA_C_CAMBRIDGE_GRIDFREE'},
                {'name': 'GGA_X_CAMA_GRIDFREE'},
            ],
    ...
    def nspin(self):
        if self._nspin is None:
            for key, val in self.out_parser.get('control_options', {}).get(
                'key_val', []
            ):
                if key == 'MULT':
    ...
    def parse_method(self):
        ...
        for key in ['basis', 'control', 'system']:
            setattr(
                sec_method,
                'x_gamess_%s_options' % key,
                {
                    key: val
                    for key, val in self.out_parser.get('%s_options' % key, {}).get(
                        'key_val', []
                    )
                },
            )
        ...
        def resolve_basis(gbasis):
            ...
            plus = ''
            for orb in ['SP', 'P']:
                if sec_method.x_gamess_basis_options.get(
                    f'DIFF{orb}', 'NONE'
                ).startswith('T'):
                    plus += '+'
            ...
            for orb in ['d', 'f', 'p']:
                ...
                orb_suffix += (
                    f',{prefix}' if orb == 'p' and orb_suffix and prefix else prefix
                )
        ...
        methods = [
            sec_method.x_gamess_control_options.get(key, 'NONE')
            for key in ['CITYP', 'CCTYP', 'VBTYP', 'TDDFT']
        ]
        methods = [m for m in methods if m != 'NONE']
        electronic_method = (
            scf_type if scf_type != 'NONE' else methods[0] if methods else run_type
        )
    ...
    def parse_calculation(self, source):
        time_initial = (
            self.archive.run[-1].calculation[-1].time_physical
            if self.archive.run[-1].calculation
            else 0 * ureg.s
        )
        sec_scc = Calculation()
        ...
        if source.scf is not None:
            ...
            if eigenvalues is not None:
                ...
                sec_eigenvalues.occupations = np.reshape(
                    np.array([2 if energy <= 0.0 else 0 for energy in eigenvalues])
                    / self.nspin,
                    shape,
                )
        ...
        if properties is not None:
            energies = {
                key.strip().lower(): val
                for key, val in properties.get('energy_components', [])
            }
            sec_energy = Energy()
            ...
            for key, val in energies.items():
                ...
                if key.endswith('energy'):
                    ...
                    if hasattr(Energy, key) or key.startswith('x_gamess'):
                        setattr(
                            ...
                            EnergyEntry(
                                ...
                                potential=potential * ureg.hartree
                                if potential
                                else potential,
                    ...
                    setattr(
                        sec_energy, self._metainfo_map.get(key, f'x_gamess_{key}'), val
                    )
            ...
            if population is not None:
                ...
                for n, method in enumerate(methods):
                    ...
                    if population.atomic is not None:
                        sec_charges.value = (
                            population.atomic[n * 2 + 1] * ureg.elementary_charge
                        )
                    if population.atomic_orbitals is not None:
                        for norbital, orbital in enumerate(
                            population.atomic_orbitals[1]
                        ):
                            sec_orbital_charges = ChargesValue()
                            sec_charges.orbital_projected.append(sec_orbital_charges)
                            sec_orbital_charges.atom_index = population.atomic_orbitals[
                                0
                            ][norbital]
                            sec_orbital_charges.orbital = str(orbital)
                            sec_orbital_charges.value = (
                                float(population.atomic_orbitals[2 + n][norbital])
                ...
                for nspin, charges in enumerate(
                    population.get('spherical_harmonics', [])[:2]
                ):
                    for norbital in range(len(charges[:-1])):
                        for natom in range(len(charges[norbital])):
                            sec_orbital_charges = ChargesValue()
                            sec_scc.charges[0].orbital_projected.append(
                                sec_orbital_charges
                            )
                            ...
                            sec_orbital_charges.value = (
                                charges[norbital][natom] * ureg.elementary_charge
                            )
    ...
    def parse_system(self, source):
        ...
        if coordinates is not None:
            ...
            sec_system.atoms = Atoms(
                labels=[
                    chemical_symbols[int(lc[1])]
                    for lc in coordinates.get('label_charge', [])
                ],
                positions=np.array(
                    [p for p in coordinates.get('position', [])], dtype=np.float64
                )
                * unit,
            )
