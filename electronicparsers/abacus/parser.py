...
from runschema.system import System, Atoms, Symmetry
...
class ABACUSOutParser(TextParser):
    ...
    def init_quantities(self):
        ...
        def str_to_sticks(val_in):
            ...
            data = []
            for v in val:
                data.append(Data(proc=int(v[0]), columns=int(v[1]), pw=int(v[2])))
            return data
        ...
        def str_to_energy_occupation(val_in):
            def extract_data(val_in, nks):
                ...
                for i in range(nks):
                    ...
                    occupations = res[:, 2]
                    state = State(
                        kpoint=np.array([kx, ky, kz], dtype=float),
                        energies=energies.astype(float),
                        occupations=occupations.astype(float),
                        npws=int(npws),
                    )
                    data.append(state)
                return data
...
class ABACUSParser:
    ...
    def parse_configurations(self):
        ...
        def parse_bandstructure(section):
            ...
            sec_k_band_segment.energies = (
                np.reshape(band_energies, (nspin, -1, nbands)) * ureg.eV
            )
        ...
        def parse_scf(iteration):
            ...
            # energies
            sec_scf.x_abacus_energy_total_harris_foulkes_estimate = (
                iteration.x_abacus_energy_total_harris_foulkes_estimate
            )
        ...
        def parse_system():
            ...
            if symmetry:
                ...
                if ibrav:
                    ...
                    if ibrav in [4, 14]:
                        sec_system_sym.bravais_lattice = brav_dict[
                            sec_system_sym.crystal_system
                        ]
                    else:
    ...
    def parse_method(self):
        ...
        if xc is not None:
            ...
            sec_xc_func = XCFunctional()
            sec_dft.xc_functional = sec_xc_func
