    ...
    def parse(self, filepath, archive, logger):
        ...
        def parse_system(source):
            ...
            sec_system.atoms = Atoms(
                ...
                positions=np.array([val[2:5] for val in source.atom_coordinates_forces])
                * ureg.bohr,
            )
        ...
        def parse_calculation(source):
            ...
            for n, scf in enumerate(source.get('scf', [])):
                ...
                sec_scf.time_physical = (
                    sec_calc.scf_iteration[n - 1].time_physical if n else time_initial
                ) + sec_scf.time_calculation
                sec_scf.energy = Energy(
                    total=EnergyEntry(value=scf[3] * ureg.hartree),
                    change=scf[4] * ureg.hartree,
                )

            if source.atom_coordinates_forces is not None:
                sec_calc.forces = Forces(
                    total=ForcesEntry(
                        value=[val[5:8] for val in source.atom_coordinates_forces]
                        * ureg.hartree
                        / ureg.bohr
                    )
                )
        ...
        if simulation_type == 'SINGLE POINT DENSITY OPTIMIZATION':
            ...
            def read_xyz_trajectory(filename):
                ...
                if trajectory:
                    ...
                    trajectory = np.reshape(
                        trajectory, (len(trajectory), len(trajectory[0]), 1, 3)
                    )
                return trajectory
            ...
            if trajectory_files:
                self.trajectory_parser.mainfile = os.path.join(
                    self.maindir, trajectory_files[0]
                )
            ...
            if trajectory is None:
                ...
                if self.trajectory_parser.data is None and trajectory_files:
                    ...
                    if trajectory is not None:
                        ...
                        trajectory = np.reshape(
                            trajectory, (n_frames, n_atoms, n_data, 3)
                        )
            ...
            for n_frame in range(len(trajectory)):
                ...
                if len(trajectory[n_frame][0]) > 1:
                    sec_system.atoms.velocities = (
                        trajectory[n_frame, :, 1, :]
                        * ureg.bohr
                        / (ureg.dirac_constant / ureg.hartree)
                    )

                time_initial = (
                    sec_run.calculation[-1].time_physical if n_frame else 0 * ureg.s
                )
                sec_calc = Calculation()
                sec_run.calculation.append(sec_calc)
                if len(trajectory[n_frame][0]) > 2:
                    sec_calc.forces = (
                        trajectory[n_frame, :, 1, :] * ureg.hartree / ureg.bohr
                    )
        ...
        sec_method.x_cpmd_simulation_parameters = (
            self.mainfile_parser.get_simulation_parameters()
        )
