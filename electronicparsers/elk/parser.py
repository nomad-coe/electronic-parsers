        ...
        self._quantities = [
            ...
            Quantity(
                ...
                sub_parser=TextParser(
                    quantities=[
                        Quantity(
                            'atom_label', r'Species \: +\d+ +\((\w+)\)', dtype=str
                        ),
                        Quantity(
                            'x_elk_muffin_tin_radius',
                            rf'muffin-tin radius \: +({re_f})',
                            dtype=np.float64,
                            unit=ureg.bohr,
                        ),
                        Quantity(
            ...
            Quantity('x_elk_lmaxapw', r'APW functions +\: +(\d+)', dtype=np.int32),
            Quantity(
            ...
            Quantity(
                'x_elk_core_charge',
                rf'Total core charge +\: +({re_f})',
                dtype=np.float64,
            ),
            Quantity(
                'x_elk_valence_charge',
                rf'Total valence charge +\: +({re_f})',
                dtype=np.float64,
            ),
            Quantity(
                'x_elk_excess_charge',
                rf'Total excess charge +\: +({re_f})',
                dtype=np.float64,
            ),
            Quantity(
            ...
            Quantity(
                'x_elk_wigner_radius',
                rf'Effective Wigner radius, r_s +\: +({re_f})',
                dtype=np.float64,
            ),
            Quantity(
                'x_elk_empty_states',
                r'Number of empty states +\: +(\d+)',
                dtype=np.int32,
            ),
            Quantity(
                'x_elk_valence_states',
                r'Total number of valence states +\: +(\d+)',
                dtype=np.int32,
            ),
            Quantity(
                'x_elk_core_states',
                r'Total number of core states +\: +(\d+)',
                dtype=np.int32,
            ),
            Quantity(
                'x_elk_lo',
                r'Total number of local\-orbitals +\: +(\d+)',
                dtype=np.int32,
            ),
            Quantity(
                'smearing_type', r'Smearing type +\: +(\d+)\s+\S+', dtype=np.int32
            ),
            Quantity(
                'smearing_width', rf'Smearing width +\: +({re_f})', dtype=np.float64
            ),
            Quantity(
            ...
            Quantity(
                'xc_functional',
                r'Exchange\-correlation functional +\: +(\d+)',
                dtype=np.int32,
            ),
            Quantity(
                'scf',
                                        ...
                                        sub_parser=TextParser(
                                            quantities=[
                                                Quantity(
                                                    ...
                                                    dtype=np.float64,
                                                    repeats=True,
                                                    str_operation=lambda x: x.rsplit(
                                                        ' ', 1
                                                    ),
                                                )
                                            ]
...
class ElkParser:
    ...
    def parse(self, filepath, archive, logger):
        ...
        lattice_vectors = (
            self.mainfile_parser.get('lattice_vectors', np.eye(3)) * ureg.bohr
        )
        ...
        for loop in self.mainfile_parser.get('scf', {}).get('loop', []):
            ...
            # energies
            energies = {
                key: val * ureg.hartree
                for key, val in loop.get('energies', {}).get('key_val', [])
            }
                ...
                if key.lower() == 'fermi':
                    ...
                    if name.endswith('_kinetic'):
                        ...
                        if energy_entry is None:
                            setattr(
                                scf_iteration.energy, name, EnergyEntry(kinetic=val)
                            )
                        else:
                        ...
                        if energy_entry is None:
                            setattr(
                                scf_iteration.energy, name, EnergyEntry(potential=val)
                            )
                        else:
                            energy_entry.potential = val
                    else:
                        val = (
                            val if name.startswith('x_elk') else EnergyEntry(value=val)
                        )
            ...
            if loop.charges is not None:
                ...
                for key, val in loop.charges.get('key_val', []):
                    ...
                    if key.lower() == 'total charge':
                        ...
                        sec_charges.value = (
                            loop.charges.get('muffin_tin', []) * ureg.elementary_charge
                        )
                    elif name is not None:
