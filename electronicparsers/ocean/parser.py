...
from runschema.method import Method, KMesh, Photon, Screening, BSE, CoreHoleSpectra
from runschema.calculation import Calculation, Spectra
from simulationworkflowschema import SinglePoint
from .metainfo.ocean import (
    x_ocean_bse_parameters,
    x_ocean_screen_parameters,
    x_ocean_core_haydock_parameters,
...
class LanczosParser(TextParser):
    ...
    def init_quantities(self):
        self._quantities = [Quantity('data', r'(.*[\s\S]+?)(?:.)', repeats=True)]
...
class OceanParser(BeyondDFTWorkflowsParser):
    def __init__(self):
        ...
        self._dft_code_map = {'qe': 'parsers/quantumespresso', 'abi': 'parsers/abinit'}
        self._bse_solver_map = {'haydock': 'Lanczos-Haydock', 'gmres': 'GMRES'}
        self.mode_bse = ['emission', 'absorption']
        self._core_level_map = {'[1, 0]': 'K', '[2, 1]': 'L23'}
    ...
    def parse_system(self, path, data):
        ...
        if data.get('bvecs'):
            sec_atoms.lattice_vectors_reciprocal = (
                np.array(data.get('bvecs')) / ureg.bohr
            )

        if data.get('znucl') and data.get('typat'):
            sec_atoms.labels = [
                chemical_symbols[int(data.get('znucl')[n_at - 1])]
                for n_at in data.get('typat')
            ]
        if data.get('xangst'):
    ...
    def parse_polarization(self, path):
        ...
        photon_file = [
            ...
            if f.startswith('photon') and f.endswith(path[-1:])
        ]
    ...
    def parse_method(self, archive):
        ...
        screen_keys = [
            ...
            'inversionstyle',
            'kshift',
            'mimic_exciting_bands',
            'shells',
        ]
        screen_dicts = ['core_offset', 'final', 'grid']
        ...
        for key in screen_dicts:
            for subkey in screen_data[key].keys():
                setattr(
                    sec_bse_screen,
                    f'x_ocean_{key}_{subkey}',
                    screen_data[key].get(subkey),
                )
        ...
        if bse_core_data.get('solver') == 'haydock':
            ...
            sec_haydock.x_ocean_converge_spacing = bse_core_data['haydock'][
                'converge'
            ].get('spacing')
            sec_haydock.x_ocean_converge_thresh = bse_core_data['haydock'][
                'converge'
            ].get('thresh')
    ...
    def parse_scc(self, path):
        ...
        lanc_file = [
            f
            for f in os.listdir(self.maindir)
            if f.startswith('abslanc') and f.endswith(path[-2:])
        ]
        if len(lanc_file) == 0:
            return
    ...
    def parse_photons(self, path):
        ...
        sec_program.x_ocean_original_dft_code = self._dft_code_map.get(
            self.data['dft'].get('program')
        )
    ...
    def get_mainfile_keys(self, **kwargs):
        ...
        absspct_files = [
            f for f in os.listdir(os.path.dirname(filepath)) if f.startswith('absspct')
        ]
        absspct_files.sort()
    ...
    def parse(self, filepath, archive, logger):
        ...
        if self._child_archives.get(child):
            if (
                self._child_archives.get(child).run[-1].program
                and self._child_archives.get(child).run[-1].system
            ):
                sec_run.program = self._child_archives.get(child).run[-1].program
                sec_run.system = self._child_archives.get(child).run[-1].system
        else:
            self.logger.warning(
                'Cannot resolve program and system from the first photon archive. '
