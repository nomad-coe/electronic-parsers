        ...
        def str_to_eigenvalues(val_in):
            ...
            for val in val_in.strip().split('\n'):
                if 'occupied orbitals' in val:
                    ...
                    if len(val) % 3 != 0:
                        continue
                    orbital_energies[-1][0].extend(
                        [val[i] for i in range(2, len(val), 3)]
                    )
                    orbital_energies[-1][1].extend(
                        [float(val[i]) for i in range(1, len(val), 3)]
                    )
        ...
        if source.dft_potential is not None:
            ...
            if len(xc_functional.exchange) == 0 and len(xc_functional.correlation) == 0:
                ...
                if name is not None:
                    ...
                    if name is not None:
                        xc_functional.contributions.append(
                            Functional(name=name, parameters=parameters)
                        )
            ...
            if source.dft_potential.molecular_quadrature is not None:
                dft.x_psi4_molecular_quadrature = (
                    source.dft_potential.molecular_quadrature[1]
                )
    ...
    def parse_calculation(self, source):
        ...
        for n in range(len(source.get('energy', []))):
            ...
            for key, val in source.energy[n].get('key_value'):
                ...
                if hasattr(ref_calc.energy, key):
                    ...
                    ref_calc.energy.contributions.append(
        ...
        if source.orbital_energies is not None:
            ...
            for n, orbital_energies in enumerate(source.orbital_energies):
                ...
                occupations.extend(
                    [2 / n_spin if n % 2 == 0 else 0] * len(orbital_energies[0])
                )
            eigenvalues.orbital_labels = labels
            eigenvalues.energies = (
                np.reshape(energies, (n_spin, 1, len(energies) // n_spin))
                * ureg.hartree
            )
            eigenvalues.occupations = np.reshape(
                occupations, (n_spin, 1, len(occupations) // n_spin)
            )
            ...
            if module.salc is not None:
                ...
                if module.salc.total_gradient is not None:
                    forces.total = ForcesEntry(
                        value=np.array(module.salc.total_gradient)
                        * ureg.hartree
                        / ureg.bohr
                    )
                for contribution in module.salc.get('contributions_gradient', []):
                    forces.contributions.append(
                        ForcesEntry(
                            ...
                            value=contribution.value * ureg.hartree / ureg.bohr,
                        )
                    )
