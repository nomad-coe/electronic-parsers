    ...
    def init_quantities(self):
        ...
        def str_to_atomic_info(val_in):
            ...
            if vals[0][0] == '1':
                ...
                info = dict(
                    coordinates=np.array(vals[-3:].T, dtype=float) * ureg.bohr,
                    atom=vals[1],
                )
            else:
                keys = vals[0][2:]
        ...
        def str_to_hessian(val_in):
            ...
            return (
                np.transpose(hessian, axes=(0, 2, 1, 3)) * ureg.hartree / ureg.bohr**2
            )
        ...
        def str_to_normal_modes_vibrational_frequencies(val_in):
            ...
            return dict(
                ...
                mode_energies=frequencies,
                infrared_activity=infrared,
        ...
        def str_to_energy_gradient(val_in):
            ...
            for val in val_in.strip().split('\n'):
                ...
                gradients[row_n].extend(
                    [
                        v.replace('D', 'E').replace('d', 'e')
                        for v in re_value.findall(row.group(2))
                    ]
                )
        ...
        def str_to_iteration(val_in):
            ...
            val = np.transpose(
                np.array([v for v in val if len(v) == len(keys)], dtype=float)
            )
            return [
                {key: val[n][i] * units.get(key, 1.0) for n, key in enumerate(keys)}
        ...
        def str_to_embedding_coordinates(val_in):
            ...
            return dict(
                label=label,
                coordinates=np.array(val[1:4].T, dtype=float) * ureg.bohr,
        ...
        def str_to_energies(val_in):
            ...
            for val in val_in.strip().split('\n'):
                ...
                if val:
                    ...
                    energies[key] = (
                        float(val.group(2)) * ureg.hartree
                        if 'energy' in key
                        else float(val.group(2))
                    )
            return energies
        ...
        def str_to_qp_states(val_in):
            val = [v.split() for v in val_in.strip().split('\n')]
            val = np.transpose(
                np.array([v[1:] for v in val if len(v) == 9], dtype=float)
            )
            keys = [
                'eigenvalue_ks_GroundState',
                'eigenvalue_quasiParticle_energy',
                'eigenvalue_ExchangeCorrelation_perturbativeGW',
                'eigenvalue_ExactExchange_perturbativeGW',
                'eigenvalue_correlation_perturbativeGW',
                'eigenvalue_ks_ExchangeCorrelation',
                'Z_factor',
                'ExchangeCorrelation_perturbativeGW_derivation',
            ]
            states = dict()
            for n, key in enumerate(keys):
                states[key] = val[n]
        ...
        def str_to_thermodynamics(val_in):
            ...
            units = {
                't': ureg.K,
                'p': ureg.MPa,
                'chem.pot.': ureg.kJ / MOL,
                'energy': ureg.kJ / MOL,
                'entropy': ureg.kJ / MOL,
                'cv': ureg.kJ / ureg.K / MOL,
                'cp': ureg.kJ / ureg.K / MOL,
        ...
        iteration_quantities = [
            Quantity(
                ...
                str_operation=lambda x: 1.0 / (1.0 + float(x)),
                convert=False,
            ),
            Quantity(
                'energies',
                r'ATION\s*ENERGY.+\s*(\d*\s*.+)',
                str_operation=str_to_scf_energies,
                convert=False,
            ),
            Quantity(
                'energy_XC', rf'Exc\s*=\s*({re_float})', dtype=float, unit=ureg.hartree
            ),
            Quantity(
                'norm_diis_scf_iteration',
                rf'Norm of current diis error\s*:\s*({re_float})',
                dtype=float,
            ),
            Quantity(
                'norm_fia',
                rf'max\. resid\. (norm for Fia\-block=\s*{re_float})\s*for orbital\s*(\w+)',
                str_operation=str_to_norm,
                convert=False,
            ),
            Quantity(
                'norm_fock',
        ...
        module_quantities = [
                ...
                sub_parser=TextParser(
                    quantities=[
                        Quantity(
                            'auxiliary',
                            r'(\w*) (?:basis|BASIS)',
                            str_operation=lambda x: 'auxiliary' in x.lower(),
                            convert=False,
                        ),
                        Quantity(
                            'spherical',
            ...
            Quantity(
                ...
                sub_parser=TextParser(
                    quantities=[
                        Quantity(
                            'pceem_max_multipole',
                            r'Maximum multipole moment used\s*:\s*(\d+)',
                            dtype=int,
                        ),
                        Quantity(
                            'pceem_multipole_precision',
                            rf'Multipole precision parameter\s*:\s*({re_float})',
                            dtype=float,
                        ),
                        Quantity(
                            'pceem_min_separation_cells',
                            rf'Minimum separation between cells\s*:\s*({re_float})',
                            dtype=float,
                        ),
                        Quantity(
                            'lattice_vectors',
                        ...
                        Quantity(
                            ...
                            str_operation=str_to_embedding_coordinates,
                            convert=False,
                        ),
                        Quantity(
                            'pc_cluster',
                ...
                sub_parser=TextParser(
                    quantities=[
                        ...
                        Quantity(
                            ...
                            rf'Edisp /kcal,au:\s*\S+\s*({re_float})',
                            dtype=float,
                            unit=ureg.hartree,
                        ),
                    ]
            ...
            Quantity(
                ...
                sub_parser=TextParser(
                    quantities=[
                        Quantity(
                            'kind', r'(\w+) smearing', str_operation=lambda x: x.lower()
                        ),
                        Quantity(
                            'width',
                            rf'Final electron temperature:\s*({re_float})',
                            dtype=float,
                        ),
                    ]
                ),
            ...
            Quantity(
                ...
                rf'Energy of reference wave function is\s*({re_float})',
                dtype=float,
            ),
            Quantity(
                'energies',
            ...
            Quantity(
                ...
                sub_parser=TextParser(
                    quantities=[
                        ...
                        Quantity(
                            'convergence',
                            r'(?:converged in (\d+) iterations|within (\d+) cycles)',
                            dtype=int,
                        ),
                    ]
                ),
            ...
            Quantity(
                'energies_MP2',
                r'(RHF\s*energy[\s\S]+?SOS\-MP2 energy.+)',
                str_operation=str_to_energies,
            ),
            Quantity(
                'energies_CCSD',
                r'(RHF\s*energy[\s\S]+?Final CCSD energy.*)',
                str_operation=str_to_energies,
            ),
            Quantity(
                'energies_CCSD(T0)',
                r'(RHF\s*energy[\s\S]+?Final CCSD\(T0\) energy.+)',
                str_operation=str_to_energies,
            ),
            Quantity(
                'energy_gradient',
                ...
                str_operation=str_to_energy_gradient,
                convert=False,
            ),
            Quantity(
                'eigenvalue_file',
                r'orbitals \$\S+\s*will be written to file (\w+)',
                repeats=True,
            ),
            Quantity(
                'wall_time',
                rf'total\s*wall\-time\s*:(.+)',
                str_operation=str_to_time,
                convert=False,
            ),
        ]
        ...
        aoforce_quantities = module_quantities + [
            ...
            Quantity(
                ...
                str_operation=str_to_normal_modes_vibrational_frequencies,
                convert=False,
            ),
            Quantity(
                'normal_modes_file',
            ...
            Quantity(
                'energy_zero_point',
                rf'\*\s*zero point VIBRATIONAL energy\s*:\s*({re_float})',
                dtype=float,
                unit=ureg.hartree,
            ),
            Quantity(
                'energy_total',
                rf'\*\s*SCF\-energy\s*:\s*({re_float})',
                drype=float,
                unit=ureg.hartree,
            ),
            Quantity(
                'energy_current',
                rf'\*\s*SCF \+ E\(vib0\)\s*:\s*({re_float})',
                dtype=float,
                unit=ureg.hartree,
            ),
        ]
                ...
                sub_parser=TextParser(
                    quantities=[
                        ...
                        Quantity(
                            ...
                            r'([\s\S]+?)\n\s*\n',
                            repeats=True,
                            str_operation=str_to_qp_states,
                            convert=False,
                        ),
                    ]
                ),
        ...
        freeh_quantities = module_quantities + [
            Quantity(
                ...
                r'(T\s*[Pp]\s*.+\n)[\s\S]+?(\d[\d\.\- ]+)',
                repeats=True,
                str_operation=str_to_thermodynamics,
                convert=False,
            ),
            Quantity(
                'energy_zero_point',
        ...
        pnoccsd_quantities = module_quantities + [
            ...
            Quantity(
                'energies',
                r'(RHF\s*energy[\s\S]+?Final CCSD\(T\) energy.+)',
                str_operation=str_to_energies,
            ),
        ]
        ...
        ricc2_quantities = ccsdf12_quantities + [
            Quantity(
                'energy_total',
                rf'Total\s+Energy\s*:\s*({re_float})',
                dtype=float,
                unit=ureg.eV,
            )
        ]
        ...
        statpt_quantities = module_quantities + [
            Quantity(
                ...
                sub_parser=TextParser(
                    quantities=[
                        Quantity('info', r'([\s\S]+)', str_operation=str_to_atomic_info)
                    ]
                ),
            ),
            Quantity(
        ...
        run_quantities = [
            Quantity(
                'x_turbomole_nodename', r'\w+\s*\((.+)\)\s*:\s*TURBO', flatten=False
            ),
            Quantity(
                'program_version',
                r'MOLE (?:V|rev\.)( *[\d\.]+\s*\([\d ]+\))',
...
class TurbomoleParser:
    def __init__(self):
        ...
        self.metainfo_map = {
            ...
            'virial theorem': 'x_turbomole_virial_theorem',
            'wavefunction norm': 'x_turbomole_wave_func_norm',
            'Final CCSD(T) energy': 'energy_total',
            'CCSD correlation energy': 'energy_current',
            'Final CCSD(T0) energy': 'energy_total',
            'Final CCSD energy': 'energy_total',
            'correlation energy': 'energy_current',
            'Final MP2 energy': 'energy_total',
            'MP2 correlation energy (doubles)': 'energy_current',
            r'HXX\+RIRPA total energy': 'energy_total',
            'RIRPA correlation energy': 'energy_current',
            'HXX total energy': 'energy_total',
            'rpa response function': 'x_turbomole_gw_use_rpa_response',
            ...
            't': 'temperature',
            'p': 'pressure',
            'entropy': 'energy_correction_entropy',
            'energy': 'energy_total',
            'cv': 'heat_capacity_c_v',
            'cp': 'heat_capacity_c_p',
        }
    ...
    def parse_system(self):
        ...
        cluster_description = {
            ...
            'pc_cluster': 'removed point charge cluster',
            'qm_cluster': 'shifted embedded QM cluster',
        }
    ...
    def parse_scc(self):
        ...
        for name, energies in self.module.items():
            ...
            for key, val in energies.items():
                ...
                if key is None:
                    continue
                if key.startswith('energy_'):
                    sec_scc_module_energy.m_add_sub_section(
                        getattr(Energy, key.replace('energy_', '').lower()),
                        EnergyEntry(value=val),
                    )
                else:
        ...
        if thermodynamics is not None:
            current_t, current_p = 0.0, 0.0
            # sec_scc_thermo = None
            for thermo in thermodynamics:
                if current_t != thermo.get('t', 0.0) or current_p != thermo.get(
                    'p', 0.0
                ):
                ...
                current_t, current_p = thermo.get('t', 0.0), thermo.get('p', 0.0)
                if sec_scc_thermo is not None:
                    for key, val in thermo.items():
                        key = self.metainfo_map.get(key, key)
                        if key.startswith('energy_'):
                            sec_scc_thermo_energy.m_add_sub_section(
                                getattr(Energy, key.replace('energy_', '').lower()),
                                EnergyEntry(value=val),
                            )
                        else:
        ...
        if self.module.get('hessian') is not None:
            ...
            # read if from file
            self.matrix_parser.mainfile = os.path.join(
                self.maindir, self.module.get('hessian_file')
            )
            try:
                hessian = np.reshape(
                    np.array(self.matrix_parser.matrix, dtype=float),
                    (n_atoms, 3, n_atoms, 3),
                )
                hessian = (
        ...
        if vibration is not None:
            ...
            if self.module.get('normal_modes_file') is not None:
                # read from file
                self.matrix_parser.mainfile = os.path.join(
                    self.maindir, self.module.get('normal_modes_file')
                )
                try:
                    normal_modes = np.array(self.matrix_parser.matrix, dtype=float)
                    sec_scc.x_turbomole_vibrations_normal_modes = np.transpose(
                        np.reshape(normal_modes, (n_atoms, 3, len(normal_modes[0]))),
                        axes=(2, 0, 1),
                    )
                except Exception:
            ...
            if self.module.get('vibrational_spectrum_file') is not None:
                self.vibrational_parser.mainfile = os.path.join(
                    self.maindir, self.module.get('vibrational_spectrum_file', '')
                )
                try:
                    ...
                    sec_scc.x_turbomole_vibrations_infrared_activity = [
                        s[-2] == 'YES' for s in spectrum
                    ]
                    sec_scc.x_turbomole_vibrations_raman_activity = [
                        s[-1] == 'YES' for s in spectrum
                    ]
        ...
        if eigenvalue_files is not None:
            ...
            try:
                for eigenvalue_file in eigenvalue_files:
                    self.eigenvalues_parser.mainfile = os.path.join(
                        self.maindir, eigenvalue_file
                    )
                    eigenvalues.append(
                        np.hstack(self.eigenvalues_parser.get('eigenvalues', []))
                    )
                    occupation.append(
                        np.hstack(self.eigenvalues_parser.get('occupation', []))
                    )
                    ...
                    values = (
                        np.reshape(
                            np.array(eigenvalues, dtype=float),
                            (len(eigenvalue_files), 1, len(eigenvalues[0])),
                        )
                        * ureg.hartree
                    )
                    occupations = np.reshape(
                        np.array(occupation, dtype=float),
                        (len(eigenvalue_files), 1, len(occupation[0])),
                    )
                    irrep = np.reshape(
                        np.array(irrep, dtype=np.dtype(np.int32)),
                        (len(eigenvalue_files), 1, len(irrep[0])),
                    )
        ...
        energies = [0.0] + [
            iteration.get('energies', {}).get('energy_total_scf_iteration')
            for iteration in self_consistency.get('iteration', [])
        ]
        for n, iteration in enumerate(self_consistency.get('iteration', [])):
            ...
            if iteration.get('time'):
                ...
                sec_iteration.time_physical = (
                    time_initial_scf + sec_iteration.time_calculation
                )
            ...
            for key in scf_keys:
                ...
                if key == 'norm_fia' or key == 'norm_fock':
                    setattr(
                        sec_iteration,
                        'x_turbomole_%s_orbital_scf_iteration' % key,
                        val.get('%s_orbital' % key),
                    )
                    val = val.get(key)
        ...
        if self.module.get('gw') is not None:
            ...
            for key, name in gw_metainfo_map.items():
                ...
                val = np.reshape(
                    val, (1, len(self.module.gw.get('qp_states', [])), len(val[0]))
                )
    ...
    def parse_method(self):
        ...
        if dft_functional is not None:
            for functional in self.xc_functional_map.get(
                dft_functional.get('functional'), ['HF_X']
            ):
            ...
            if dft_functional.exchange is not None:
                sec_method.x_turbomole_functional_type_exchange = (
                    dft_functional.exchange
                )

            if dft_functional.correlation is not None:
                sec_method.x_turbomole_functional_type_correlation = (
                    dft_functional.correlation
                )
        ...
        for key, val in self.control_parser.items():
            ...
            if key == 'drvopt':
                for drvopt_key, drvopt_val in val.items():
                    setattr(
                        sec_method,
                        'x_turbomole_controlIn_%s_status' % drvopt_key,
                        drvopt_val,
                    )
            else:
    ...
    def parse(self, filepath, archive, logger):
        ...
        if len(time) > 0:
            start = datetime.strptime(
                time[0][0], '%Y-%m-%d %H:%M:%S.%f'
            ) - datetime.utcfromtimestamp(0)
            end = datetime.strptime(
                time[-1][1], '%Y-%m-%d %H:%M:%S.%f'
            ) - datetime.utcfromtimestamp(0)
            sec_run.time_run = TimeRun(
                date_start=start.total_seconds(), date_end=end.total_seconds()
            )
