...
class AbinitParser(BeyondDFTWorkflowsParser):
    ...
    def parse_regex_into_json(self, source):
        source = source.results  # json with all the keys/values of TextParser
        parameters = {}
        for keys in source.keys():
            value = source.get(keys)
            if isinstance(value, list):
                parameters_list = []
                for val in value:
                    if hasattr(val, 'quantities'):
                        parameters_list.append(self.parse_regex_into_json(val))
                    else:
                        parameters_list.append(val)
                parameters[keys] = parameters_list
            else:
                if hasattr(value, 'quantities'):
                    parameters[keys] = self.parse_regex_into_json(value)
                else:
    ...
    def parse_gw_datasets(self):
        ...
        for step in ['screening', 'gw']:
            step_dataset = self.out_parser.get(f'{step}_dataset')
