...
from .metainfo.abinit import (
    ...
    x_abinit_mesh,
    x_abinit_gw_dataset,
    x_abinit_ks_band_gaps_params,
    x_abinit_qp_band_gaps_params,
    x_abinit_sigma_params,
...
class AbinitParser(BeyondDFTWorkflowsParser):
            ...
            for key in self.out_parser.energy_components.keys():
                ...
                if key == 'internal':
                    ...
                    sec_energy.m_add_sub_section(
                        getattr(Energy, key), EnergyEntry(value=energy)
                    )
                ...
                for i in self._selfenergy_data_map.keys():
                    value = np.array(
                        [
                            [
                                selfenergy_data[:, i]
                                for s in range(sec_k_eigen.n_spin_channels)
                            ]
                            for k in range(sec_k_eigen.n_kpoints)
                        ]
                    )
                    value = value * ureg.eV
    ...
    def parse(self, filepath, archive, logger):
        ...
        if (
            ...
            and len(optdriver) == 4
            and (optdriver[-1] == 4 or optdriver[-1] == 66)
        ):
            # GW single point
            p = AbinitParser()
