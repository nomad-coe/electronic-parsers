...
class InpParser(ControlParser):
    ...
    def init_quantities(self):
        ...
        super().init_quantities()
        self._quantities += [
            Quantity('block', r'%([\s\S]+?)%', repeats=True, str_operation=str_to_block)
        ]
...
class OctopusParser:
    ...
    def init_parser(self, filepath, logger):
        ...
        self.eigenvalue_parser.mainfile = os.path.join(
            self.maindir, 'static/eigenvalues'
        )
    ...
    def parse_scc(self):
        ...
        for minimization in self.out_parser.get('minimization', []):
            ...
            if number is not None:
                path = os.path.join(
                    self.out_parser.maindir, 'geom/go.%04d.xyz' % number
                )
                try:
        ...
        if len(sec_run.calculation) > 0:
            ...
            for key, val in energies.items():
                ...
                if metainfo_key.startswith('energy_'):
                    sec_energy.m_add_sub_section(
                        getattr(Energy, metainfo_key.replace('energy_', '').lower()),
            ...
            # eigenvalues
            eigenvalues = self.eigenvalue_parser.get(
                'eigenvalues', self.info_parser.get('eigenvalues')
            )
            if eigenvalues is not None and eigenvalues.get('eigenvalues') is not None:
                kpts, eigs, occs = list(
                    zip(*[e for e in eigenvalues.eigenvalues if e is not None])
                )
                ...
                eigs = eigs * self._units_mapping.get(
                    self.info.get('energyunit').lower()
                )
    ...
    def parse_method(self):
        ...
        # smearing
        smearing_function = self._smearing_functions.get(
            self.info.get('SmearingFunction'), None
        )
        if smearing_function is not None:
            ...
            if smearing_width is not None:
                sec_electronic.smearing.width = (
                    (
                        smearing_width
                        * self._units_mapping.get(self.info['energyunit'].lower())
                    )
                    .to('joule')
                    .magnitude
                )
        ...
        if dft_u is not None:
            ...
            theory_level = self._theory_levels.get(
                self.info.get('TheoryLevel', 4), None
            )
    ...
    def parse(self, filepath, archive, logger):
        ...
        sec_run.program = Program(
            name='Octopus', version=str(header.get('Version', ''))
        )
