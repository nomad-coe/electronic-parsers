...
from .metainfo.onetep import x_onetep_section_tddft, x_onetep_section_tddft_excitations
            ...
            for block in self.get('block', []):
                if block[0].lower() == 'lattice_cart':
                    ...
                    cell['lattice_vectors'] = (
                        block[1] * ureg.bohr if unit is None else block[1][1:] * unit
                    )
                    ...
                    cell['positions'] = (
                        np.array([v[1:4] for v in value], dtype=np.dtype(np.float64))
                        * unit
                    )
...
class OutParser(TextParser):
    def init_quantities(self):
            ...
            Quantity(
                ...
                sub_parser=TextParser(
                    quantities=[
                        ...
                        Quantity(
                            'labels', r'([A-Z][a-z]*) +\d+', dtype=str, repeats=True
                        ),
                    ]
                ),
            ),
        ...
        self._quantities = [
            ...
            Quantity(
                'program_name', r'(\S+) is based on developments described', dtype=str
            ),
            Quantity(
                'x_onetep_number_of_processors',
                r'Default threads: (\d+)',
                dtype=np.dtype(np.int32),
            ),
            Quantity(
                'input_file', r'Reading parameters from file \"(.+?)\"', dtype=str
            ),
            Quantity(
                'input',
            ...
            Quantity(
                ...
                sub_parser=TextParser(
                    quantities=[
                        ...
                        Quantity(
                            'converged',
                            r'Geometry +optimization +(\S+)',
                            str_operation=lambda x: x == 'completed',
                        ),
                        Quantity(
                            'final_configuration',
...
class OnetepParser:
    def __init__(self):
        ...
        self._metainfo_map = {
            'Kinetic': 'kinetic_electronic',
            'Hartree': 'correction_hartree',
    ...
    def parse_configuration(self, source: TextParser):
        ...
        def parse_system(source):
            ...
            positions = (
                positions * ureg.bohr if isinstance(positions, list) else positions
            )
            sec_system.atoms = Atoms(positions=positions, labels=cell.get('labels'))
            sec_system.atoms.lattice_vectors = cell.get(
                'lattice_vectors', self.input_parser.cell.get('lattice_vectors')
            )
            return sec_system
        ...
        def parse_scc(source):
            ...
            for iteration in source.get('iteration', []):
                ...
                sec_scf.energy = Energy(total=EnergyEntry(value=iteration.energy_total))
                ...
                for contribution in energies.get('contribution', []):
                    ...
                    if key is not None:
                        setattr(
                            sec_energy,
                            key,
                            EnergyEntry(value=contribution[1] * ureg.hartree),
                        )
            elif len(sec_scc.scf_iteration) > 0:
                sec_scc.energy = Energy(
                    total=EnergyEntry(
                        value=sec_scc.scf_iteration[-1].energy.total.value
                    )
                )
            ...
            if source.get('mulliken') is not None:
                ...
                sec_charges.value = [
                    v[3] for v in source.get('mulliken', [])
    ...
    def parse(self, filepath, archive, logger):
        ...
        sec_run.program = Program(
            name=self.out_parser.get('program_name'),
        ...
        self.input_parser.mainfile = os.path.join(
            self.maindir, self.out_parser.get('input_file', '')
        )

        input_parameters = {
            key: val
            for key, val in self.out_parser.get('input', {}).get('parameter', [])
        }
        sec_method = Method()
        ...
        if input_parameters.get('xc_functional') is not None:
            ...
            for xc_functional in self._xc_map.get(
                input_parameters.get('xc_functional'), []
            ):
        ...
        if self.out_parser.geometry_optimization is not None:
            if self.out_parser.geometry_optimization.single_point is not None:
                self.parse_configuration(
                    self.out_parser.geometry_optimization.single_point
                )
            for iteration in self.out_parser.geometry_optimization.get('iteration', []):
                self.parse_configuration(
                    iteration if iteration.improve is None else iteration.improve
                )
            ...
            if self.out_parser.tddft is not None:
                ...
                for excitation in self.out_parser.tddft.get('excitation', []):
                    ...
                    sec_excitation.x_onetep_tddft_excit_energy = (
                        excitation[1] * ureg.hartree
                    )
                    sec_excitation.x_onetep_tddft_excit_oscill_str = excitation[2]
                    sec_excitation.x_onetep_tddft_excit_lifetime = (
                        excitation[3] * ureg.ns
                    )
