...
def test_scf(parser):
    ...
    assert sec_method.scf.x_band_diis_settings_dirac['Method'] == 'diis'
    assert sec_method.scf.x_band_diis_settings_scf['Cond'] == approx(1.0e+6)
    assert not sec_method.scf.x_band_degenerate
    assert sec_method.scf.x_band_growth_factor == approx(1.1)
    assert sec_method.scf.x_band_convrg == approx(1.0e-6)
    assert sec_method.x_band_nuclear_charge_density_model == 'Point Charge Nuclei'
    assert sec_method.x_band_grimme_d3_dispersion_correction['damping'] == 'BJ'
    assert sec_method.x_band_grimme_d3_dispersion_correction['s8'] == approx(1.944)
    assert sec_method.x_band_other_parameters['version'] == 4
    ...
    assert sec_atom[0].x_band_radial_points == 3000
    assert sec_atom[1].x_band_nuclear_charge == approx(1.0)
    assert sec_atom[0].n_valence_electrons == 4
    assert sec_atom[1].charge == approx(0.)
    assert sec_atom[0].orbitals[1] == '2S'
    assert sec_atom[1].x_band_orbital_energies[0].magnitude == approx(-1.01788704e-18)
    assert sec_atom[0].x_band_orbital_radii[2] == approx(1.794)
    assert sec_atom[1].x_band_energy_sum_eigenvalues.magnitude == approx(-1.01800039e-18)
    assert sec_atom[0].x_band_energy_kinetic.magnitude == approx(1.62379128e-16)
    assert sec_atom[1].x_band_n_radial_fit_functions == 0
    assert sec_atom[0].label == 'C'
    assert sec_atom[1].x_band_cutoff_valence == approx(23.76)
    assert sec_atom[0].x_band_cutoff_core_kinetic == approx(4.36)
    assert sec_method.x_band_run_config['Zlm fitting']
    ...
    assert sec_scc.eigenvalues[0].x_band_energy_min[0][3].magnitude == approx(-2.51827575e-18)
    assert sec_scc.eigenvalues[0].x_band_energy_max[1][16].magnitude == approx(-2.32112809e-19)
    assert sec_scc.eigenvalues[0].x_band_occupations[1][14] == 0.
    ...
    assert sec_scc.eigenvalues[0].band_gap[0].x_band_conduction_band_index == 15
...
def test_geometry_optimization(parser):
    ...
    assert sec_sccs[19].energy.x_band_fit_error_correction.value.magnitude == approx(-8.0916862e-23)
    assert sec_sccs[0].energy.x_band_v_atomic_def.value.magnitude == approx(-3.6046467e-18)
    assert sec_sccs[2].energy.x_band_dispersion.value.magnitude == approx(-4.7877583e-20)
    assert len(sec_sccs[6].scf_iteration) == 11
    assert sec_sccs[10].forces.total.value[5][1].magnitude == approx(3.11423748e-11)
    assert sec_sccs[20].forces.x_band_dispersion.value[9][0].magnitude == approx(-6.5909788e-12)
    assert sec_sccs[10].forces.x_band_pair_interactions.value[1][2].magnitude == approx(8.9338246e-09)
    assert sec_sccs[8].forces.x_band_electrostatic.value[6][1].magnitude == approx(-4.2076891e-08)
    assert sec_sccs[9].forces.x_band_xc.value[3][2].magnitude == approx(3.04008897e-11)
    assert sec_sccs[5].forces.x_band_electronic_kinetic.value[8][2].magnitude == approx(-3.4639301e-08)
    assert sec_sccs[1].forces.x_band_p_matrix.value[4][0].magnitude == approx(-8.88908833e-09)
    assert sec_sccs[4].eigenvalues[0].x_band_energy_min[0][15].magnitude == approx(-2.84386148e-19)
    ...
    assert not sec_methods[16].x_band_run_config['Store original Bloch functions']
    assert sec_methods[21].atom_parameters[1].x_band_cutoff_valence_kinetic == approx(24.05)
...
def test_geometry_optimization_new(parser):
    archive = EntryArchive()
    parser.parse('tests/data/band/EDUSIF.out', archive, None)
    ...
    assert sec_scc[-1].energy.x_band_dispersion.value.magnitude == approx(-1.92033353e-18)
    assert sec_scc[-1].forces.total.value[163][0].magnitude == approx(-1.43919212e-11)
    assert sec_scc[-1].forces.x_band_electronic.value[7][2].magnitude == approx(-2.99480631e-09)
    assert sec_scc[-1].forces.x_band_electrostatic.value[291][1].magnitude == approx(9.48766784e-11)
    assert sec_scc[-1].forces.x_band_nuclear_repulsion.value[3][2].magnitude == approx(-4.79941725e-09)
    assert sec_scc[-1].forces.x_band_dispersion.value[423][1].magnitude == approx(-1.1331013e-11)
    ...
    assert sec_scc[-1].eigenvalues[0].x_band_energy_min[0][11].magnitude == approx(-3.34244189e-18)
    assert sec_scc[-1].eigenvalues[0].x_band_occupations[0][775] == approx(2.0)
