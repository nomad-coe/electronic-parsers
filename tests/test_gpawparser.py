...
def test_gpw(parser):
    ...
    assert sec_method.electronic.smearing.width == approx(0.0)
    ...
    assert sec_scc.x_gpaw_fixed_spin_Sz == approx(0.0)
...
def test_gpw2(parser):
    ...
    assert sec_scc.eigenvalues[0].occupations[0][0][0] == approx(1.0)
    assert sec_scc.calculation_converged
...
def test_spinpol(parser):
    ...
    assert archive.run[0].calculation[0].x_gpaw_magnetic_moments[0] == approx(1.0)
