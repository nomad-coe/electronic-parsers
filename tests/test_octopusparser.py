...
def test_scf(parser):
    ...
    assert sec_run.x_octopus_parserlog_SpeciesProjectorSphereThreshold == approx(0.001)
...
def test_spinpol(parser):
    ...
    assert sec_run.x_octopus_parserlog_PreconditionerFilterFactor == approx(0.6)
    ...
    assert sec_eig.kpoints[9][0] == approx(0.5)
    assert sec_eig.occupations[0][1][17] == approx(0.972222)
    ...
    assert sec_scfs[8].time_calculation.magnitude == approx(15.63)
