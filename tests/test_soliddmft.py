...
def test_srvo3(parser):
    ...
    # input DMFT
    assert isinstance(sec_run.method[1].x_soliddmft_general_params, dict) and len(sec_run.method[1].x_soliddmft_general_params) == 48
    ...
    assert sec_scc.energy.chemical_potential.to('eV').magnitude == approx(0.050287075199847694)
    assert len(sec_scc.energy.double_counting.values_per_atom.to('eV').magnitude) == 1
    assert sec_scc.energy.double_counting.values_per_atom.to('eV').magnitude[0] == approx(7.544843542949807e-05)
...
def test_lacu2o4_real_freq(parser):
    ...
    assert sec_run.program.x_soliddmft_hash == '707c012359758d3aca1ce46e3446312df86ca5d6'
