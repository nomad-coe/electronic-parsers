...
def test_vasprunxml_static(parser):
    ...
    assert sec_scc.dos_electronic[0].total[0].value[1838].magnitude == approx(.1369 / ureg.eV.to('joule').magnitude)
...
def test_outcar(parser):
    ...
    assert sec_dos.total[0].value[282].magnitude == approx(.2545E+01 / ureg.eV.to('joule').magnitude)
