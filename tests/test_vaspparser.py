...
def test_dftu_static(parser):
    ...
    Test data generated by N. Daelman on JENA cluster using VASP 6.1.0"""
...
def test_corehole(parser):
    """Tests the core hole localization.
    """
    archive = EntryArchive()
    reference_file = 'tests/data/vasp/NaTiO_core_hole/vasprun.xml'
    parser.parse(reference_file, archive, None)
    sec_method = archive.run[-1].method[-1]

    # Check the k-point sampling
    k_mesh = sec_method.k_mesh
    assert list(k_mesh.grid) == [2, 2, 3]
    assert k_mesh.sampling_method == "Gamma-centered"

    # Check core hole description
    sec_core_hole = sec_method.atom_parameters[0].core_hole
    assert sec_core_hole.n_quantum_number == 1
    assert sec_core_hole.l_quantum_number == 0
    assert sec_core_hole.occupancy == 0.
